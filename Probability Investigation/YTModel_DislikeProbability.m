...
% This function runs an investigation on the probability of disliking
...
interval = .0005; % the amount dislike_prob is increased each iteration
...
% initialize dislike_prob vector
                ...
                if rand > 1-p % with a p percent chance
                    ...
                    if rand > 1-sub_prob % with a sub_prob chance
                        ...
                        if rand > 1-like_ns_prob % like with a like_ns_prob chance
                            liked = liked+1;
                        elseif rand > 1-dislike_prob(k) % or dislike with a dislike_prob chance
                            disliked = disliked+1;
                        end
...
% Graph all statistics against dislike_prob
figure()
...
% Graph subs, comments and dislikes against dislike_prob
figure()
plot(dislike_prob,total_subs, 'r')
hold on
xlabel('dislike prob')
...
% Graph dislikes against dislike_prob
figure()
