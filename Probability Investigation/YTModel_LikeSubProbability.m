function YTModel_LikeSubProbability
%YTMODEL_LikeSubProbability
% This function runs an investigation on the probability of liking if 
...
interval = .1; % the amount like_sub_prob is increased each iteration
...
% initialize like_sub_prob vector
like_sub_prob=zeros(Niterations,1);
like_sub_prob(1)=.1;
for k = 2:Niterations
    like_sub_prob(k) = like_sub_prob(k-1)+interval;
end
    ...
    for i=1:Ndays
        ...
        for j=1:Nusers
            if subscribed(j) % if the user is subscribed
                ...
                if rand > 1-like_sub_prob(k) % with like_sub_prob chance
                ...
                if rand > 1-p % with a p percent chance
                    ...
                    if rand > 1-sub_prob % with a sub_prob chance
                        ...
                        subscribed(j)=1;
                        subbed=subbed+1;
                        if rand > 1-like_sub_prob(k) % like with a like_sub_prob chance
                            liked = liked+1;
                        end
...
% Graph all statistics against like_sub_prob
figure()
...
% Graph subs, comments and dislikes against like_sub_prob
figure()
plot(like_sub_prob,total_subs, 'r')
hold on
xlabel('like subbed prob')
...
% Graph dislikes against like_sub_prob
figure()
title('Investigation of Like (Subbed) Probability: Dislikes')
plot(like_sub_prob,total_dislikes, 'y')
