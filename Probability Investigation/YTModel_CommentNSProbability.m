function YTModel_CommentNSProbability
%YTMODEL_CommentNSProbability
% This function runs an investigation on the probability of commenting if
% not subscribed in YTModel_OneRun. Each other probability is held as a
...
interval = .00025; % the amount comment_ns_prob is increased each iteration

% Data Tracking (for the line graphs)
...
% initialize comment_ns_prob vector
comment_ns_prob=zeros(Niterations,1);
comment_ns_prob(1)=.00025;
for k = 2:Niterations
    comment_ns_prob(k) = comment_ns_prob(k-1)+interval;
end
            ...
            if subscribed(j) % if the user is subscribed
                ...
                if rand > 1-p % with a p percent chance
                    ...
                    if rand > 1-sub_prob % with a sub_prob chance
                        ...
                        if rand > 1-comment_ns_prob(k) % comment with a comment_ns_chance
...
% Graph all statistics against comment_ns_prob
figure()
...
% Graph subs, comments and dislikes against comment_ns_prob
figure()
plot(comment_ns_prob,total_subs, 'r')
hold on
xlabel('comment not subbed prob')
...
% Graph dislikes against comment_ns_prob
figure()
title('Investigation of Comment (Not Subbed) Probability: Dislikes')
plot(comment_ns_prob,total_dislikes, 'y')
