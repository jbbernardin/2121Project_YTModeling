function YTModel_ViewExponential
%YTMODEL_ViewExponential
% This function runs an investigation on the exponential base for 
% viewing in YTModel_OneRun. Each other base is held as a
    ...
    for i=1:Ndays
        ...
        for j=1:Nusers
            if subscribed(j) % if the user is subscribed
                ...
                p = min(recommmend_prob*a(k)^viewed*b(mid)^liked*c(mid)^subbed*d(mid)^commented,max_p); % only a varies
...
plot(a,total_views, 'k')
plot(a,total_likes, 'b')
plot(a,total_comments, 'g')
plot(a,total_dislikes, 'y')
hold off

% Graph a against subs, comments and dislikes
figure()
plot(a,total_subs, 'r')
hold on
xlabel('a')
